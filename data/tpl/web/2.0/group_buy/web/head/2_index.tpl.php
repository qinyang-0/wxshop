        					...
        					<form action="./index.php" method="get" class="form-horizontal" role="form" id="form1">
								...
								<div class="am-u-sm-12 ">
									<div class="am-form-group tpl-form-border-form zx-form-input zx-display zx-group">
										<input type="text" class="am-form-field" name="key" placeholder="编号/用户名/姓名/电话" value="<?php  echo $_GPC['num'];?>" style="border-radius: 4px;width: 240px;">
									</div>
							...
							<div class="am-scrollable-horizontal am-u-sm-12">
								<table width="100%" class="am-table am-table-compact am-table-striped tpl-table-black am-text-nowrap">
									...
									<tr>
										...
										<td class="am-text-middle"><img src="<?php  echo $item['m_photo'];?>" width="50" style="border-radius: 50%;"/>
										...
										<td class="am-text-middle" style="cursor:pointer;" onclick="commission(<?php  echo $item['m_id'];?>,this)"><?php  echo $item['m_money'];?></td>
										...
										<td class="am-text-middle">
											<div class="tpl-switch">
												<input type="checkbox" class="ios-switch bigswitch tpl-switch-btn" name="type" data-id="<?php  echo $item['m_id'];?>" data-type="closes" data-state="<?php  if($item['closes']==2) { ?>1<?php  } else { ?>2<?php  } ?>" <?php  if($item['closes']==2) { ?> checked="" <?php  } ?>>
												<div class="tpl-switch-btn-view">
													<div>
...
<script type="text/javascript">
	$(function () {
        ...
        $(document).on("change","input[name='type']",function (res) {
            ...
            var data = $(this).data();
            $.post("<?php  echo $this->createWebUrl('head',array('op'=>'state'))?>", {
                id: data.id,
                val: data.state,
                type: data.type,
            }, function (result) {
    ...
    function commission(id,obj){
    	console.log(id);
    	$.post("<?php  echo $this->createWebUrl('head',array('op'=>'commission_save'))?>",{id:id},function(res){
			var str = res;
			if(str == 1){
				layer.msg('用户不存在');
				return false;
				...
				layer.open({
					title: '改变佣金(<span style="color:red;">请谨慎修改，一但出错，概不负责</span>)',
					content: str,
					area:['800px', '480px'],
					yes:function(index,layers){
						...
						data['token'] = 'submit';
						layer.confirm('确定变更佣金，一但变更将有可能导致后面的佣金不一致!!!', {icon: 2, title:'提示'}, function(index){
							layer.load(0, {shade: [0.5, '#000000']}); //又换了种风格，并且设定最长等待10秒 
							$.post("<?php  echo $this->createWebUrl('head',array('op'=>'commission_save'))?>",data,function(res){
								console.log(res);
								if(res.code == 2){
									layer.msg(res.msg);
								}else{
