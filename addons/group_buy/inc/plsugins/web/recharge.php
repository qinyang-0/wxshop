...
switch($in){
	case 'index':
		...
		$total= pdo_fetchcolumn('select count(*) from ' . tablename('gpb_recharge')." where status = 1 and recharge_type = 2 and weid=".$weid);
		$page = pagination($total,$pageIndex,$pageSize);
		//获取分页信息
		$sql = 'select * from '.tablename('gpb_recharge')." where status = 1 and recharge_type = 2 and weid=".$weid." order by weight desc ".$contion;
...
//		if(checksubmit('submit')){
	...
	case 'bannel_add':
		...
		if($post){
			...
			$this->message_info('操作成功', $this->createWebUrl('plsugins',['op'=>'recharge','in'=>'bannel']), 'success');
		}else{
...
//				$card = pdo_getall("gpb_member_card_time",array('weid'=>$this->weid,'status'=>1,'c_id'=>$id), array() , '' , array('id','asc'));
	...
	case 'markrting_recharge':
		...
		$title = $_GPC['title'];
		if($title){
			$where .= " and m.m_nickname like '%".trim($title)."%'";
		}
		$index=isset($_GPC['page'])?$_GPC['page']:1;
        $pageIndex = $index;
        $pageSize = 10;
		$contion = 'limit '.($pageIndex-1) * $pageSize .','. $pageSize;
		$total= pdo_fetchcolumn('select count(*) from ' . tablename('gpb_recharge_info')." i join ".tablename("gpb_member")." m on i.openid = m.m_openid where i.status = 1 and i.recharge_type = 2 and i.weid=".$weid." and i.pay_status = 20".$where);
		$page = pagination($total,$pageIndex,$pageSize);
		//获取分页信息
		$sql = 'select i.*,m.m_nickname,m.m_photo from '.tablename('gpb_recharge_info')." i join ".tablename("gpb_member")." m on i.openid = m.m_openid where i.status = 1 and i.weid=".$weid." and i.recharge_type = 2 and i.pay_status = 20 ".$where." order by time desc ".$contion;
		$info = pdo_fetchall($sql);
		if(is_array($info)){
		    foreach ($info as $k=>$v){
	...
	case 'gold':
		...
		$type = $_GPC['type'];
		if($title){
			$where .= " and m.m_nickname like '%".$title."%'";
		}
		if($type){
