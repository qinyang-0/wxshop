...
switch($in){
	...
	case 'card_add':
		...
		if($post){
			...
			$this->message_info('操作成功', $this->createWebUrl('plsugins',['op'=>'card']), 'success');
		}else{
...
//				$card = pdo_getall("gpb_member_card_time",array('weid'=>$this->weid,'status'=>1,'c_id'=>$id), array() , '' , array('id','asc'));
	...
	case 'card_order':
        ...
        $pageSize = 10;
		$contion = 'limit '.($pageIndex-1) * $pageSize .','. $pageSize;
		$total= pdo_fetchcolumn('select count(*) from ' . tablename('gpb_member_card_order')." i join ".tablename("gpb_member")." m on i.openid = m.m_openid where  i.weid=".$weid." and i.card_status = 20".$where);
		$page = pagination($total,$pageIndex,$pageSize);
		//获取分页信息
		$sql = 'select i.*,m.m_nickname,m.m_photo,m.m_phone from '.tablename('gpb_member_card_order')." i join ".tablename("gpb_member")." m on i.openid = m.m_openid where  i.weid=".$weid." and i.card_status = 20 ".$where." order by id desc ".$contion;
		$info = pdo_fetchall($sql);
		if($info){
			foreach($info as $k=>$v){
				if($v['m_nickname']){
