...
//            }
			...
			if(empty($res)){
                ...
                $this->message_info('操作成功', $this->createWebUrl('market'), 'success');
			}
		}else{
		    $act_title="新增";
			if($id){
...
//            }
            ...
            pdo_commit();
            if(!empty($res)){
                $this->message_info("设置成功",$this->createWebUrl('market',array('op'=>'new_member')), 'success');
            }else{
...
if($do == 'market'){
    $do = 'action';
    include $this -> template('web/' . 'market' . '/' . $op);
}else{
