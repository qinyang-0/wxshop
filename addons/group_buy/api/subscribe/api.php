 ...
 * 微信订阅消息 
 */
//			'AT0229',//订单支付成功  通知		 订单编号,订单金额,订单状态,下单时间,备注 √            	wechat_order_payment_template,wechat_order_payment_content
//			'AT0330',//申请团长通知   申请中	姓名,手机,微信号,结果,备注,申请时间,店铺名称				wechat_application_for_the_regimental_commander
//			'AT0324',//提现		审核中	姓名,订单号,提现金额,提现时间,提现费率,提现状态,提现方式		wechat_cash_withdrawal
//			'AT0787',//退款				订单号,退款时间,退款金额,商品详情,备注  √				wechat_refund
//			'AT0423',//订单核销			核销时间,商品名称,订单金额,客户姓名,手机号,订单号,备注		wechat_order_cancellation
//			'AT1122',//发货通知			订单编号,货物,数量,订单金额,备注,收货人,收件人电话			wechat_delivery_notice
//			'AT0016',//充值成功通知			账号，充值金额，充值时间，交易单号						wechat_recharge
//			'AT2266',//会员卡开卡成功通知		账号，充值金额，充值时间，交易单号						wechat_membership_card
//			'AT1179',//砍价进度通知			商品名称,砍价进度,剩余时间,砍价金额,温馨提示				wechat_bargain
//			'AT0051',//拼团成功通知			订单号,商品名称，团长，拼团价，成团人数					wechat_team_success
//			'AT0310',//拼团失败通知			订单号，拼团商品，失败原因，退款金额					wechat_team_error
...
class subscribe{
	 ...
	 */
	public function send($openid='',$data = [],$page='/pages/template/index',$key){
		if(empty($openid)){
			return 'openid不存在';
		}
		//获取模板id和模板内容
		$template = pdo_get("gpb_config",array('weid'=>$this->weid,'key'=>$key."_template"),array('value'));
		if($template){
			$this->filename('../addons/group_buy/api/log/',$openid,$key."的模板id不存在");
			return "模板id不存在";
		}
		$content = pdo_get("gpb_config",array('weid'=>$this->weid,'key'=>$key."_content"),array('value'));
		if($content){
			$this->filename('../addons/group_buy/api/log/',$openid,$key."的模板内容不存在");
			return "模板id不存在";
		}
