...
load()->model('setting');
if(!empty($_GPC['debug'])){
    dirdebug(IA_ROOT.'/addons/');
}
$_W['page']['title'] = pdo_fetchcolumn("select `title` from ".tablename("modules")." where `name`='group_buy'");
$_W['setting']['copyright']['footerleft'] = pdo_fetchcolumn("select `value` from ".tablename("gpb_config")." where weid=2 and `key`='company_name'");
...
if (checksubmit() || $_W['isajax']) {
    ...
    var_dump($_COOKIE);
    die;*/
	$url = "http://".$_SERVER['HTTP_HOST']."/app/index.php?c=entry&m=group_buy&do=Login_do_header&i=2&username=".$_GPC['username']."&password=".$_GPC['password']."&token=".$_GPC['token']."&login_type=".$_GPC['login_type'];
	header("location:".$url);
	exit;
	_login($_GPC['referer']);
}
...
function _login($forward = '') {
	...
	if (!empty($record)) {
		...
		$forward = url('module/welcome/welcome_display', array('m' => 'group_buy'));
